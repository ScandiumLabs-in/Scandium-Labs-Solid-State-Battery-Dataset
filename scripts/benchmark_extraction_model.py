...
def load_ground_truth() -> list[dict]:
    ...
    for it in q.get("items", []):
        if it.get("status") != "approved":
            continue
        pdf = it.get("pdf_path") or it.get("evidence", {}).get("pdf")
...
def score_extraction(records: list, gt_by_pdf: dict[str, list[dict]]) -> dict:
    """Compare extracted records against per-PDF ground truth."""
    n_sigma_expect = sum(1 for g in gt_by_pdf.values() for g in g
                         if g.get("property") in ("sigma", "conductivity"))
    n_ea_expect = sum(1 for g in gt_by_pdf.values() for g in g
                      if g.get("property") == "activation_energy")
    sigma_hits = ea_hits = 0
    by_composition: dict[str, int] = {}
    for rec in records:
        comp = (rec.get("composition") or rec.identity.composition
                if hasattr(rec, "identity") else rec.get("composition", ""))
        # pull the canonical values off the record
        sig = getattr(rec, "sigma", None)
        if sig is None and hasattr(rec, "ion_transport"):
            sig = rec.ion_transport.sigma_S_per_cm
        ea = getattr(rec, "ea_eV", None)
        if ea is None and hasattr(rec, "ion_transport"):
            ea = rec.ion_transport.Ea_eV
        ...
        if sig is not None and any(
            ...
            for g in gt_by_pdf.get(key, []) if g.get("property") in ("sigma", "conductivity")
        ):
            sigma_hits += 1
        if ea is not None and any(
            abs(ea - g.get("value")) <= 0.05
            for g in gt_by_pdf.get(key, []) if g.get("property") == "activation_energy"
        ):
            ea_hits += 1
    return {
...
def main() -> int:
    ...
    if args.determinism:
        ...
        max_assign = assignments.most_common(1)
        print(f"determinism({args.determinism} runs, model={args.model}):")
        print(f"  record counts: {counts} → {'STABLE' if stable else 'UNSTABLE'}")
        print(f"  most frequent (composition, sigma): {max_assign[0][0]} ×{max_assign[0][1]}/{args.determinism}")
        if args.persist:
    ...
    for pdf in pdfs:
        ...
        sc = score_extraction(recs, {Path(pdf).name: by_pdf[pdf]})
